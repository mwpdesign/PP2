...
def upgrade() -> None:
    ...
    op.add_column(
        "patients",
        sa.Column("skilled_nursing_facility", sa.Boolean(), server_default="false"),
    )
    op.add_column(
        "patients",
        sa.Column("part_a_stay_coverage", sa.Boolean(), server_default="false"),
    )
    ...
    # Create indexes
    op.create_index(
        "ix_patient_documents_patient_id", "patient_documents", ["patient_id"]
    )
    op.create_index("ix_patient_documents_type", "patient_documents", ["document_type"])
    op.create_index(
        "ix_patient_documents_category", "patient_documents", ["document_category"]
    )
    op.create_index(
        "ix_secondary_insurance_patient_id", "secondary_insurance", ["patient_id"]
    )
    op.create_index(
        "ix_secondary_insurance_provider", "secondary_insurance", ["insurance_provider"]
    )
...
def downgrade() -> None:
    ...
    # Drop RLS policies
    op.execute(
        "DROP POLICY IF EXISTS patient_documents_isolation_policy ON patient_documents"
    )
    op.execute(
        "DROP POLICY IF EXISTS secondary_insurance_isolation_policy ON secondary_insurance"
    )
