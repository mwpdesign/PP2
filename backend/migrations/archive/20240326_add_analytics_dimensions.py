...
Revision ID: 20240326_add_analytics_dimensions
Revises: 20240328_create_core_tables
...
revision = "20240326_add_analytics_dimensions"
down_revision = "20240328_create_core_tables"
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    op.create_index(
        "idx_satisfaction_level", "dim_patient_satisfaction", ["satisfaction_level"]
    )
    op.create_index(
        "idx_feedback_category", "dim_patient_satisfaction", ["feedback_category"]
    )
    ...
    op.create_index(
        "idx_verification_type", "dim_verification_performance", ["verification_type"]
    )
    op.create_index(
        "idx_sla_category", "dim_verification_performance", ["sla_category"]
    )

    # Add new columns to fact_calls
    op.add_column("fact_calls", sa.Column("satisfaction_id", sa.Integer()))
    op.add_column("fact_calls", sa.Column("verification_performance_id", sa.Integer()))
    op.add_column("fact_calls", sa.Column("sentiment_score", sa.Float()))
    op.add_column("fact_calls", sa.Column("feedback_text", sa.String(500)))
    ...
    op.create_index("idx_calls_satisfaction", "fact_calls", ["satisfaction_id"])
    op.create_index(
        "idx_calls_verification", "fact_calls", ["verification_performance_id"]
    )
    ...
    op.add_column(
        "agg_daily_metrics",
        sa.Column(
            "satisfaction_by_category", JSONB, nullable=False, server_default="{}"
        ),
    )
    op.add_column(
        "agg_daily_metrics",
        sa.Column("sentiment_trend", JSONB, nullable=False, server_default="{}"),
    )
    op.add_column(
        "agg_daily_metrics",
        sa.Column("feedback_categories", JSONB, nullable=False, server_default="{}"),
    )
    op.add_column(
        "agg_daily_metrics",
        sa.Column("verification_by_type", JSONB, nullable=False, server_default="{}"),
    )
    op.add_column(
        "agg_daily_metrics",
        sa.Column("sla_performance", JSONB, nullable=False, server_default="{}"),
    )
    op.add_column(
        "agg_daily_metrics",
        sa.Column("error_distribution", JSONB, nullable=False, server_default="{}"),
    )
