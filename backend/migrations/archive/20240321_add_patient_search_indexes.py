...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_patients_territory_org", "patients", ["territory_id", "organization_id"]
    )
    ...
    op.create_index(
        "ix_patients_insurance_verified", "patients", ["insurance_verified"]
    )
    ...
    op.create_index("ix_patients_consent_status", "patients", ["consent_status"])

    # Create composite index for provider/facility filtering
    op.create_index(
        "ix_patients_provider_facility", "patients", ["provider_id", "facility_id"]
    )

    # Create index for medical record number (encrypted)
    op.create_index(
        "ix_patients_mrn_encrypted", "patients", ["medical_record_number_encrypted"]
    )
