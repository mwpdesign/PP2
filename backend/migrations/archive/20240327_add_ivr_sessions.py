...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("insurance_data", postgresql.JSONB, nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("insurance_coverage", postgresql.JSONB, nullable=True),
        sa.Column(
            "created_at", sa.DateTime, nullable=False, server_default=sa.text("now()")
        ),
        sa.Column(
    ...
    # Create indexes
    op.create_index("ix_ivr_sessions_patient_id", "ivr_sessions", ["patient_id"])
    op.create_index("ix_ivr_sessions_provider_id", "ivr_sessions", ["provider_id"])
    op.create_index("ix_ivr_sessions_territory_id", "ivr_sessions", ["territory_id"])
    op.create_index("ix_ivr_sessions_status", "ivr_sessions", ["status"])
    op.create_index(
        "ix_ivr_session_items_session_id", "ivr_session_items", ["session_id"]
    )
    op.create_index(
        "ix_ivr_session_items_product_id", "ivr_session_items", ["product_id"]
    )
