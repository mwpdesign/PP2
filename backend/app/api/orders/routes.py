...
async def create_product(
    ...
    if not current_user.get("is_admin"):
        raise HTTPException(
            status_code=403, detail="Only administrators can create products"
        )
    ...
    if product.category_ids:
        categories = (
            ...
            .filter(ProductCategory.id.in_(product.category_ids))
            .all()
        )
    ...
    await audit_product_change(db, "create", db_product.id, current_user["id"])
...
async def update_product(
    ...
    if not current_user.get("is_admin"):
        raise HTTPException(
            status_code=403, detail="Only administrators can update products"
        )
    ...
    for field, value in product.dict(exclude_unset=True).items():
        if field == "category_ids" and value is not None:
            categories = (
                db.query(ProductCategory).filter(ProductCategory.id.in_(value)).all()
            )
            db_product.categories = categories
        else:
    ...
    await audit_product_change(db, "update", product_id, current_user["id"])

    return db_product


@router.get(
    "/products/{product_id}/inventory", response_model=List[ProductInventoryResponse]
)
...
@router.post(
    "/products/{product_id}/inventory", response_model=ProductInventoryResponse
)
async def update_inventory(
    ...
    if not current_user.get("is_admin"):
        raise HTTPException(
            status_code=403, detail="Only administrators can update inventory"
        )
...
@router.get(
    "/products/{product_id}/compliance", response_model=List[ProductComplianceResponse]
)
...
async def create_category(
    ...
    if not current_user.get("is_admin"):
        raise HTTPException(
            status_code=403, detail="Only administrators can create categories"
        )
...
@router.post("/{order_id}/status", response_model=OrderStatusResponse)
...
async def update_order_approval(
    ...
    return await order_service.update_order_approval(
        order_id, approval_id, approval_data
    )


@router.get("", response_model=OrderSearchResponse)
