...
class ProductBase(BaseModel):
    ...
    regulatory_status: str = Field(..., regex='^(approved|pending|restricted)$')
...
class ProductUpdate(BaseModel):
    ...
    description: Optional[str] = Field(None, max_length=1000)
    manufacturer: Optional[str] = Field(None, max_length=200)
    regulatory_status: Optional[str] = Field(None, regex='^(approved|pending|restricted)$')
...
class ProductComplianceBase(BaseModel):
    ...
    status: str = Field(..., regex='^(compliant|pending|non-compliant)$')
...
class ProductComplianceUpdate(BaseModel):
    status: Optional[str] = Field(None, regex='^(compliant|pending|non-compliant)$')
    certification_number: Optional[str] = Field(None, max_length=100)
    certification_date: Optional[datetime] = None
    expiration_date: Optional[datetime] = None
    documentation_url: Optional[str] = Field(None, max_length=500)
...
class ProductSearchParams(BaseModel):
    ...
    compliance_status: Optional[str] = Field(
        None,
        regex='^(compliant|pending|non-compliant)$'
    )
...
class OrderUpdate(BaseModel):
    ...
    status: Optional[str] = Field(
        None,
        regex='^(pending|verified|approved|processing|completed|cancelled)$'
    )
...
class OrderApprovalBase(BaseModel):
    ...
    approval_type: str = Field(
        ...,
        regex='^(provider|insurance|pharmacy|supervisor)$'
    )
    notes: Optional[str] = Field(None, max_length=1000)
    metadata: Optional[Dict] = None
...
class OrderSearchParams(BaseModel):
    ...
    sort_by: str = Field('order_date', regex='^[a-zA-Z_]+$')
