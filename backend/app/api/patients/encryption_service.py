    ...
    async def decrypt_patient_data(
        ...
        try:
            ...
            for field, value in encrypted_data.items():
                if (
                    field in self.encrypted_fields and
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to decrypt patient data: {str(e)}"
            )
                ...
                if field in patient_data:
                    ...
                    if value is not None:
                        if not isinstance(value, dict):
                            raise ValueError(
                                f"Field {field} is not encrypted"
                            )
                        if 'encrypted_data' not in value:
                            raise ValueError(
                                f"Field {field} missing encrypted data"
                            )
                        if 'encrypted_key' not in value:
                            raise ValueError(
                                f"Field {field} missing encrypted key"
                            )
            return True
