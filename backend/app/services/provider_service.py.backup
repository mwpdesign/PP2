...
class ProviderService:
    ...
    def create_facility(self, facility_data: FacilityCreate) -> Facility:
        ...
        if facility_data.territory_id:
            if not verify_territory_access(self.current_user, facility_data.territory_id):
    ...
    def create_provider(self, provider_data: ProviderCreate) -> Provider:
        ...
        facility = self.db.query(Facility).filter_by(id=provider_data.facility_id).first()
        if not facility:
            raise NotFoundException("Facility not found")
        
        if not verify_territory_access(self.current_user, facility.territory_id):
    ...
    def update_provider_credentials(
        ...
        # Verify territory access
        if not any(
            verify_territory_access(self.current_user, t.id)
    ...
    def create_territory(self, territory_data: TerritoryCreate) -> ProviderTerritory:
        """Create a new provider territory."""
        # Verify parent territory access if specified
        if territory_data.parent_id:
            if not verify_territory_access(self.current_user, territory_data.parent_id):
    ...
    def create_provider_relationship(
        ...
        provider = self.db.query(Provider).filter_by(id=relationship_data.provider_id).first()
        ...
        if not (
            any(verify_territory_access(self.current_user, t.id) for t in provider.territories)
            and any(
                verify_territory_access(self.current_user, t.id)
                for t in related_provider.territories
            )
        ):
            raise UnauthorizedError("No access to one or both providers' territories")
    ...
    def search_providers(self, search_params: ProviderSearchParams, page: int = 1, size: int = 20) -> Dict:
        """Search providers with territory-based access control."""
        query = self.db.query(Provider)

        # Apply search filters
        if search_params.territory_id:
            if not verify_territory_access(self.current_user, search_params.territory_id):
                raise UnauthorizedError("No access to specified territory")
        ...
        if search_params.facility_id:
            query = query.filter(Provider.facility_id == search_params.facility_id)

        if search_params.specialty:
            query = query.filter(Provider.specialty == search_params.specialty)
    ...
    def verify_provider_credentials(self, credential_id: str) -> ProviderCredentials:
        """Verify provider credentials and update status."""
        credentials = self.db.query(ProviderCredentials).filter_by(id=credential_id).first()
        if not credentials:
