...
class SecurityMonitoringService:
        ...
        try:
            ...
            recent_failures = self.db.query(SecurityEvent).filter(
                and_(
                    ...
                    SecurityEvent.created_at >= datetime.utcnow() - timedelta(
                        minutes=30
                    )
                )
            ).count()
    ...
    async def detect_suspicious_patterns(
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Error detecting suspicious patterns: {str(e)}"
            )
    ...
    async def _check_cloudtrail_events(self) -> None:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Error checking CloudTrail events: {str(e)}"
            )
    ...
    async def _lock_account(self, user_id: int, reason: str) -> None:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Error locking account: {str(e)}"
            )
    ...
    async def _send_sns_alert(self, alert: SecurityAlert) -> None:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Error sending SNS alert: {str(e)}"
            )
    ...
    async def _get_security_team_members(
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Error getting security team members: {str(e)}"
            )
