...
class InventoryService:
    ...
    async def get_item_locations(
        ...
        try:
            locations = []
            for item in items:
                item_locations = self.db.query(WarehouseLocation).filter(
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to get item locations: {str(e)}"
            )
    ...
    async def add_inventory(
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to add inventory: {str(e)}"
            )
    ...
    async def remove_inventory(
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to remove inventory: {str(e)}"
            )
