...
class QueueService:
    ...
    async def receive_messages(
        self, max_messages: int = 10, visibility_timeout: int = 300
    ) -> List[Dict]:
        ...
        try:
            ...
            for message in response.get("Messages", []):
                try:
                    ...
                    decrypted_content = self.encryption_service.decrypt(message["Body"])
    ...
    async def delete_message(self, receipt_handle: str) -> bool:
        ...
        try:
            self.sqs_client.delete_message(
                QueueUrl=self.main_queue_url, ReceiptHandle=receipt_handle
            )
            return True
        ...
        try:
            ...
            for message in response.get("Messages", []):
                try:
                    ...
                    self.sqs_client.delete_message(
                        QueueUrl=self.dlq_url, ReceiptHandle=message["ReceiptHandle"]
                    )
