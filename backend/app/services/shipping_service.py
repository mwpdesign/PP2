...
class ShippingService:
    def __init__(self):
        self.carriers = {
            "ups": UPSCarrier(
                api_key=settings.UPS_API_KEY, account_number=settings.UPS_ACCOUNT_NUMBER
            ),
            "fedex": FedExProvider(
    ...
    async def create_shipping_label(self, carrier: str, shipment_info: Dict) -> Dict:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to create shipping label: {str(e)}"
            )
    ...
    async def track_shipment(self, carrier: str, tracking_number: str) -> Dict:
        ...
        try:
            ...
            carrier_service = self.carriers[carrier]
            tracking_info = await carrier_service.track_shipment(tracking_number)
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to track shipment: {str(e)}"
            )

    async def validate_address(
        self, address: Dict, carrier: Optional[str] = None
    ) -> Dict:
        """Validate shipping address."""
        try:
            validation_results = []
            carriers = [self.carriers[carrier]] if carrier else self.carriers.values()
            ...
            for carrier_service in carriers:
                try:
                    ...
                    print(
                        "Error validating address with " f"{carrier_service}: {str(e)}"
                    )
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to validate address: {str(e)}"
            )

    async def estimate_delivery_date(
        self, carrier: str, service_type: str, origin: Dict, destination: Dict
    ) -> Dict:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to estimate delivery date: {str(e)}"
            )
