...
class Provider(Base):
    ...
    tax_id: Mapped[str] = mapped_column(
        String(10), unique=True, nullable=False, comment="ENCRYPTED"
    )
    ...
    patients = relationship(
        "Patient", foreign_keys="Patient.provider_id", back_populates="provider"
    )
    orders = relationship("Order", back_populates="provider")
    credentials = relationship(
        "ProviderCredentials", back_populates="provider", cascade="all, delete-orphan"
    )

    # IVR relationships
    ivr_requests = relationship(
        "IVRRequest", back_populates="provider", cascade="all, delete-orphan"
    )
    ivr_sessions = relationship(
        "IVRSession", back_populates="provider", cascade="all, delete-orphan"
    )
...
class ProviderCredentials(Base):
    ...
    credential_type: Mapped[str] = mapped_column(
        String(100), nullable=False, comment="License, Certificate, etc."
    )
    credential_number: Mapped[str] = mapped_column(
        String(100), nullable=False, comment="ENCRYPTED"
    )
...
class ProviderRelationship(Base):
    ...
    provider = relationship(
        "Provider", foreign_keys=[provider_id], back_populates="provider_relationships"
    )
